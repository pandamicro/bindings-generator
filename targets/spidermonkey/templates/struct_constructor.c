...
static bool ${struct_constructor_name}(se::State& s)
{
    ...
    if(argc == 0) 
    {
        ${namespaced_class_name}* cobj = new (std::nothrow) ${namespaced_class_name}();
    ...
    else if(argc == ${len($public_fields)})
    {
        ...
        ${field_type} arg${arg_idx};
            #end if
        ${field.ntype.to_native({"generator": $generator,
                             "arg_type": $field.ntype,
...
SE_BIND_SUB_CLS_CTOR(${struct_constructor_name}, __jsb_${underlined_class_name}_class, js_${underlined_class_name}_finalize)
